...
case $VERSION in
    25.05|25.11|latest)
        PMIX_VERSIONS="${PMIX_VERSIONS:-6.0.0 5.0.9 4.2.9}"
        ;;
    *)
        # 6.0.0 is not compatible with Slurm <= 25.05.2
	PMIX_VERSIONS="${PMIX_VERSIONS:-5.0.9 4.2.9}"
	;;
esac
...
case $VERSION in
  ...
  24.11)
    ...
    BUILD_DEPS="json-c-devel http-parser-devel jansson-devel doxygen"
    if [ -z "$nonflight" ]; then
      TAG="flight-slurm-24-11-5-1-flight1"
      REL="flight-slurm-24.11.5.flight1"
    else
      TAG="slurm-24-11-5-1-flight1"
      REL="slurm-24.11.5.flight1"
    fi
    libjwt=true
    nvml=true
    ;;
  25.05)
    ...
    BUILD_DEPS="json-c-devel http-parser-devel jansson-devel doxygen"
    if [ -z "$nonflight" ]; then
      TAG="flight-slurm-25-05-4-1-flight1"
      REL="flight-slurm-25.05.4.flight1"
    else
      TAG="slurm-25-05-4-1-flight1"
      REL="slurm-25.05.4.flight1"
    fi
    libjwt=true
    nvml=true
    ;;
  25.11|latest)
    ...
    BUILD_DEPS="json-c-devel http-parser-devel jansson-devel doxygen"
    if [ -z "$nonflight" ]; then
      TAG="flight-slurm-25-11-0-1-flight1"
      REL="flight-slurm-25.11.0.flight1"
    else
      TAG="slurm-25-11-0-1-flight1"
      REL="slurm-25.11.0.flight1"
    fi
    libjwt=true
    nvml=true
    ;;
