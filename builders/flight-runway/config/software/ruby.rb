...
build do
  ...
  # mjt - 2019-04-10 - patch configure.ac s.t. we don't use the
  # CentoOS 7.5+ only supported "compress-debug-sections" option to
  # `ld`.
  if rhel? &&
     platform_version.satisfies?("< 8") &&
     version.satisfies?(">= 2.4")
    patch source: "remove-compress-debug-sections.patch", plevel: 0, env: patch_env
  end
